...
describe('Unit testing for ArtistCollections Component', () => {
    ...
    it('Renders correctly on shallowMount', () => {
        const wrapper = shallowMount(ArtistCollections, {
            data: function () {
                return {
                    musicRecords: testDataStructure,
                    musicRecordsFilteredInClient: testDataStructure,
                    artistName: "coltrane"
                }
            }
        });
        const hasDataNode = wrapper.find("#has-artist-data");
        expect(hasDataNode).toBeDefined();        
    });


    it('Mocks a call to the HTTP service', async () => {
        const wrapper = mount(ArtistCollections, {
            data: function () {
                return {
        ...
        wrapper.vm.getArtistRecords();
        await flushPromises();                
        expect(wrapper.vm.musicRecords[0].artistId).toBe(71822);
    });

    it('Orders data correctly when clicked', async () => {
        const wrapper = mount(ArtistCollections, {
            data: function () {
                return {
                    artistName: 'coltrane'
        ...
        wrapper.vm.getArtistRecords();
        await flushPromises();
        const elemOrderDate = wrapper.find("#order_releaseDate");
        elemOrderDate.trigger('click');
        expect(wrapper.vm.musicRecordsFilteredInClient[0].releaseDate).toBe('1962-08-01T07:00:00Z');
    });
